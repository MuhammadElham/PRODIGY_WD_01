  ...
  const BlogData = [
    {
      ...
      publish: "Jan 20, 2024 by Dilshad",
      image: Img1,
    },
    {
          ...
          {BlogData.map((data) => (
            <div key={data.title} className="bg-white dark:bg-gray-900">
                ...
                <div className="space-y-2">
                    <p className="text-xs text-gray-500">{data.publish}</p>
                    <p className="font-bold line-clamp-1">{data.title}</p>
                    <p className="line-clamp-2 text-sm text-gray-600 dark:text-gray-400">{data.substitle}</p>
                </div>
            </div>
          ))}
        </div>
